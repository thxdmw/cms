...
/**
 * {@link SystemMonitorService} 的默认实现。
 * 完全基于 JDK 自带的 {@code java.lang.management}（{@link OperatingSystemMXBean}、
 * {@link MemoryMXBean}、{@link ThreadMXBean} 等）和 {@link Runtime}/{@link File} 采集数据，
 * 不依赖任何第三方监控组件，因此拿到的是"当前 JVM 进程所在机器"的资源使用情况。
 */
@Service
public class SystemMonitorServiceImpl implements SystemMonitorService {
    ...
    /**
     * 系统资源总览：汇总 CPU / 内存 / 磁盘 / JVM / 线程信息，再加上操作系统基本信息（名称、版本、
     * 架构、处理器核数、系统负载）
     */
    @Override
    ...
    /**
     * CPU 使用率百分比：用系统平均负载（load average）除以处理器核数近似换算得到。
     * 部分平台（典型是 Windows）不支持 load average，此时返回 -1 表示"不支持/不可用"
     */
    @Override
    ...
    /**
     * 内存使用情况：既有基于 {@link Runtime} 的进程整体内存视图（总量/已用/空闲/最大），
     * 也有更细的 JVM 堆内存、非堆内存详情（init/used/committed/max）
     */
    @Override
    ...
    /**
     * 磁盘使用情况：取的是项目运行时工作目录（{@code new File(".")}）所在磁盘/分区的总空间、
     * 已用空间、剩余空间，并非机器上所有磁盘的汇总
     */
    @Override
    ...
    /**
     * JVM 信息：虚拟机名称/版本、Java 版本、厂商、启动时间、已运行时长，
     * 以及每个垃圾回收器的回收次数和累计耗时
     */
    @Override
    ...
    /**
     * 线程信息：当前线程数、峰值线程数、守护线程数、累计启动过的线程数，
     * 以及按 {@link Thread.State} 分组的线程数量分布
     */
    @Override
